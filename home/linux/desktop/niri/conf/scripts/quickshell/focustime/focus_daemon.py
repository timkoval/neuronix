...
def listen_niri_ipc():
    global current_app_class, current_app_title, _ipc_proc

    while True:
        try:
            _ipc_proc = subprocess.Popen(
                ['niri', 'msg', '--json', 'event-stream'],
                stdout=subprocess.PIPE, text=True
            )
            for line in _ipc_proc.stdout:
                try:
                    event = json.loads(line)
                    if 'WindowFocusChanged' in event or 'WindowOpenedOrChanged' in event:
...
def exit_handler(sig, frame):
    tracker.flush()
    if _ipc_proc is not None:
        _ipc_proc.terminate()
        try:
            _ipc_proc.wait(timeout=2)
        except Exception:
            _ipc_proc.kill()
    sys.exit(0)

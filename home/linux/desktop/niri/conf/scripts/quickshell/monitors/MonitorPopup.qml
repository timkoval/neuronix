                ...
                MouseArea {
                    ...
                    onClicked: {
                        ...
                        if (monitorsModel.count === 1) {
                            ...
                            Quickshell.execDetached(["sh", "-c", "niri msg output " + mon.name + " mode " + mon.resW + "x" + mon.resH + "@" + mon.rate]);
                        } else {
                            let rects = [];
                            for (let i = 0; i < monitorsModel.count; i++) {
                                let m = monitorsModel.get(i);
                            ...
                            for (let i = 0; i < rects.length; i++) {
                                ...
                                batchCmds.push("niri msg output " + r.name + " mode " + r.resW + "x" + r.resH + "@" + r.rate + " position " + r.x + " " + r.y);
                                summaryString += r.name + " ";
                            }

                            let fullCommand = batchCmds.join(" && ");
